...
public class InventoryS3Client {
	...
	protected Product[] getAllProducts() {
		...
		ResponseInputStream<?> objectData = s3Client.getObject(
				GetObjectRequest.builder().bucket("inventory-llucio-data").key("handy-tool-catalog.json").build());
	...
	protected boolean updateAllProducts(Product[] products) {
		...
		PutObjectResponse putResponse = s3Client.putObject(
				PutObjectRequest.builder().bucket("inventory-llucio-data").key("handy-tool-catalog.json").build(),
